...
public class AssociationTest extends AbstractTest {
	...
	public void spyTest() throws AranukaException {
		...
		// checking mr bond
		Person p = (Person) session.getBySubjectIdentifier(spy1
		...
		p = (Person) session
				.getBySubjectIdentifier(spy2.getSubjectIdentifier());
		...
		p = (Person) session
				.getBySubjectIdentifier(spy1.getSubjectIdentifier());
		assertEquals("James Bond", p.getName());
		assertTrue(p.isSpy());
	}
	...
	public void weaponTest() throws Exception {
		...
		desertEagle
				.setSubjectIdentifier("http://en.wikipedia.org/wiki/IMI_Desert_Eagle");
		...
		// checking bond
		Person p = (Person) session.getBySubjectIdentifier(bond
				.getSubjectIdentifier());
		...
		// checking tony
		p = (Person) session
				.getBySubjectIdentifier(tony.getSubjectIdentifier());
		...
		// rechecking bond
		p = (Person) session
				.getBySubjectIdentifier(bond.getSubjectIdentifier());
		assertEquals(bond.getName(), p.getName());
		assertEquals(bond.getGun(), p.getGun());
		assertEquals(bond.isSpy(), p.isSpy());
	...
	public void modification() throws AranukaException, IOException {
		...
		Person p2 = (Person) session.getBySubjectIdentifier(p
				.getSubjectIdentifier());
	...
	public void cascade() throws Exception {
		...
		session.clearCache();
		Company testComp = (Company) session.getBySubjectIdentifier(comp
				.getSubjectIdentifier());

		serializeTopicMap("/tmp/company.ctm", session.getTopicMap());

		assertEquals(comp.getSubjectIdentifier(),
				testComp.getSubjectIdentifier());
	...
	private void checkNewPerson(Person expected, Person actual) {
		assertEquals(expected.getSubjectIdentifier(),
				actual.getSubjectIdentifier());
